...
//     //const geometry = new THREE.distanceToSquared(CIRCLE_RADIUS);
//     const material = new THREE.MeshBasicMaterial({ color: CIRCLE_COLOR });

//     const circleMaterial = new CANNON.Material("circleMaterial");
//     circleMaterial.friction = 0.0; // Very low friction
//     circleMaterial.restitution = 0.1; // High bounciness
  
//     // Define a contact material for interactions between circles
//     const contactMaterial = new CANNON.ContactMaterial(circleMaterial, circleMaterial, {
//       friction: 0.0, // Almost no friction
...
//       let x, y, valid;
//       do {
//         x = Math.random() * (BOX_WIDTH - CIRCLE_RADIUS * 2) - BOX_WIDTH / 2;
//         y = Math.random() * (BOX_HEIGHT - CIRCLE_RADIUS * 2) - BOX_HEIGHT / 2;
//         valid = bodies.every(
//           (body) => body.position.distanceTo(new CANNON.Vec3(x, y, 0)) > CIRCLE_RADIUS * 2
//         );
//       } while (!valid);

//       // Three.js mesh
//       // material.CIRCLE_COLOR += 1220;
//       const mesh = new THREE.Mesh(geometry, material);
//       scene.add(mesh);
//       circles.push(mesh);

//       // Physics body
//       const shape = new CANNON.Sphere(CIRCLE_RADIUS);
...
//     const boundaryThickness = 70;

//     const createWall = (x: number, y: number, angle: number) => {
//       const shape = new CANNON.Box(new CANNON.Vec3(halfWidth, boundaryThickness / 2, 1));
...
//     createWall(0, halfHeight, 0); // Top
//     createWall(0, -halfHeight, 0); // Bottom
//     createWall(-halfWidth, 0, 30); // Left
//     createWall(halfWidth, 0, -30); // Right
//   };

//   const handleMouseMove = (event: MouseEvent) => {
//     const rect = canvasRef.current?.getBoundingClientRect();
//     if (!rect) return;
//     const x = event.clientX - rect.left - BOX_WIDTH / 2;
//     const y = -event.clientY + rect.top + BOX_HEIGHT / 2;
//     mousePositionRef.current = { x, y };
//   };

//   const applyRadialPush = () => {
...
//       if (distance < PUSH_RADIUS && distance > 1) {
//         const forceScale = (PUSH_RADIUS - distance) / PUSH_RADIUS;
//         const forceX = (dx / distance) * forceScale * 600000;
//         const forceY = (dy / distance) * forceScale * 300000;
...
//   return <div ref={canvasRef} className="absolute top-0 left-0  h-100% w-100%" />;
// };
...
const ParticleSimulation = () => {
  ...
  const [numParticles, setNumParticles] = useState(1200);
